...
func (db *DB) SetMaxIdleConns(n int) {
	log.Printf("[%d] SetMaxIdleConns(%d)\n", time.Now().UTC().UnixNano(), n)
	db.mu.Lock()
	if n > 0 {
		db.maxIdle = n
	} else {
...
func (db *DB) SetMaxOpenConns(n int) {
	log.Printf("[%d] SetMaxOpenConns(%d)\n", time.Now().UTC().UnixNano(), n)
	db.mu.Lock()
	db.maxOpen = n
	if n < 0 {
		db.maxOpen = 0
...
func (db *DB) maybeOpenNewConnections() {
	...
	log.Printf("[%d] maybeOpenNewConnections(); db.maxOpen= %d; numRequests= %d\n", time.Now().UnixNano(), db.maxOpen, numRequests)
	if db.maxOpen > 0 {
	...
	for numRequests > 0 {
		...
		log.Printf("[%d] request for new connection\n", time.Now().UnixNano())
		db.openerCh <- struct{}{}
	}
}
...
func (db *DB) openNewConnection() {
	log.Printf("[%d] openNewConnection()\n", time.Now().UTC().UnixNano())
...
func (db *DB) exec(ctx context.Context, query string, args []interface{}, strategy connReuseStrategy) (res Result, err error) {
	log.Printf("[%d] exec(query= %v; args= %v)\n",
		time.Now().UnixNano(),
		query,
		args,
	)
